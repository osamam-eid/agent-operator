...
export function makeCompiledWorkflow(
  ...
  return {
    ...
    decisionTrace: {
      schemaVersion: '1.0',
      entries: [
        { stage: 'CLASSIFICATION', summary: 'Fixture classification recorded.', reasonCodes: ['FIXTURE_CLASSIFIED'] },
        { stage: 'DISCLOSURE', summary: 'Fixture disclosure recorded.', reasonCodes: ['DEFAULT_NATIVE_INTENT'] },
        { stage: 'GRAPH_COMPILATION', summary: 'Fixture graph compiled.', reasonCodes: ['GRAPH_COMPILED'] },
      ],
    },
    capabilitySummaries: options.nodes.map((node) => ({
      nodeId: node.nodeId,
      role: node.role ?? 'mock-reader',
      capabilityId: 'mock-read-capability',
      provider: 'mock',
